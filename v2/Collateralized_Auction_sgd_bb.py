...
def _run_one_bb_sgd(advertisers, d, k, params0, eps_vec, lo_bounds, hi_bounds,
                    num_iterations, batch_size, lr, rng, v_scale=1.0):
    ...
    for t in range(1, num_iterations + 1):
        ...
        for ki in range(n_params):
            ...
            if abs(denom) > 1e-15:
                grad[ki] = (batch_welfare(p_plus, batch) - batch_welfare(p_minus, batch)) / denom
...
def run_bb_sgd_search(advertisers, polynomial_degree, k=1, num_iterations=500,
                      batch_size=50, lr=0.02, eps=0.1, num_restarts=5, seed=None):
    """
    ...
    print(f"BB-SGD: {d+1} points ({n_params} params), {num_iterations} iters x batch {batch_size}, {num_restarts} restarts ({n_flat} flat + {n_random} random slope)")
    ...
    for r, params0 in enumerate(init_params_list):
        ...
        best_params, best_w, traj = _run_one_bb_sgd(
            advertisers, d, k, params0, eps_vec, lo_bounds, hi_bounds,
            num_iterations, batch_size, lr, rng, v_scale=v_max,
        )

        try:
            best_coeffs = points_to_coeffs(_flat_to_points(best_params, d))
        except (np.linalg.LinAlgError, ValueError):
            print(f"  Restart {r+1}/{num_restarts}  y_init=[{y_init_str}]  [interpolation failed, skipping]")
            global_trajectory.extend(traj)
            continue

        exact_w = float(np.mean([_eval_on_auction(best_coeffs, ads, k) for ads in advertisers]))
        print(f"  Restart {r+1}/{num_restarts}  y_init=[{y_init_str}]  batch_best={best_w:.6f}  exact={exact_w:.6f}")
    ...
    print(f"BB-SGD complete in {elapsed:.1f}s  global exact welfare={global_best_w:.6f}")
...
def compile_results(externality_cost_per_impression, num_advertisers, num_auctions,
                    random_seed, k, polynomial_degree, auction_output, advertisers):
    tau_coeffs = auction_output['tau']
    _, _, _, best_coll_welfare, best_vcg_welfare, best_iw = run_auction_set(tau_coeffs, advertisers, k)
...
def main(args):
    ...
    _, best_avg_coll, best_avg_vcg, best_coll_welfares, best_vcg_welfares, best_iw = run_auction_set(
        best_coeffs, advertisers, k
    )
...
def print_usage():
    ...
    print("  --batch-size INT           Optional. Auction draws per gradient step. Defaults to 50.")
    print("  --lr FLOAT                 Optional. Adam learning rate. Defaults to 0.02.")
    print("  --eps FLOAT                Optional. Finite-difference base step size (relative to bbox). Defaults to 0.1.")
    print("  --num-restarts INT         Optional. Adam restarts sweeping initial tau level. Defaults to 5.")
...
def parse_args(argv):
    ...
    while i < len(argv):
        ...
        if arg == '--help':
            ...
            i += 1; args['num_iterations'] = int(argv[i])
        elif arg == '--batch-size':
            i += 1; args['batch_size'] = int(argv[i])
        elif arg == '--lr':
            i += 1; args['lr'] = float(argv[i])
        elif arg == '--eps':
            i += 1; args['eps'] = float(argv[i])
        elif arg == '--num-restarts':
            i += 1; args['num_restarts'] = int(argv[i])
        elif arg == '--id':
