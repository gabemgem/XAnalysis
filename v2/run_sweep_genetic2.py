...
from Collateralized_Auction_genetic2 import run_genetic_search, tau as _eval_tau
...
def simulate_auctions(bidder_sets, tau_coeffs, k):
    ...
    for bidder_set in bidder_sets:
        ...
        if len(adm_v) == 0:
            ...
            pa_payments.append(float(sorted_adm_v[k]) if len(sorted_adm_v) >= k + 1 else 0.0)
...
def run_one(data_raw, k, degree, ext_cost, action_cost,
            num_items, num_auctions,
            num_generations, sol_per_pop, num_parents_mating,
            mutation_probability, range_multiplier,
            sample_seed, seed, num_restarts=1,
            initial_solution=None,
            test_seed=None, num_test_auctions=2000):
    data = data_raw.copy()
    ...
    print(f"\n{'='*60}")
    print(f"  k={k}  degree={degree}  ext_cost={ext_cost:.6g}  "
    ...
    for restart in range(num_restarts):
        ...
        if welfare > best_welfare:
            ...
            best_coeffs  = coeffs
            if num_restarts > 1:
                print(f"  * New best  welfare={welfare:.6f}  "
    ...
    for idx, rec in enumerate(records):
        ...
        for coeffs in fns:
            ax.plot(e_line, [_eval_poly(coeffs, e) for e in e_line],
                    color='gray', alpha=0.12, linewidth=0.5, zorder=2)
...
def plot_penalty_grid(records, out_dir, tag, percentile=99):
    ...
    for idx, rec in enumerate(records):
        ...
        if not legend_done:
            ax.legend(
                [p1, p2, p3],
                ['τ (optimal threshold)', 'R(e) (participant penalty)', '−e (recipient penalty)'],
                fontsize=6, loc='best',
            )
            legend_done = True
...
def plot_welfare_comparison(records, out_dir, tag):
    ...
    ax.plot(ext_costs, vcg_means,  marker='o', linewidth=1.8,
            label='VCG (counterfactual)',        color='steelblue')
    ax.plot(ext_costs, vcga_means, marker='s', linewidth=1.8,
            label='Participant Audit (VCGA)',     color='firebrick')
    ax.plot(ext_costs, ra_means,   marker='^', linewidth=1.8,
...
def plot_tau_degree_comparison(all_results, degrees_ordered, fig_dir, k_values, num_items_list,
                               percentile=99):
    ...
    for k in k_values:
        for num_items in num_items_list:
            avail_degrees = [d for d in degrees_ordered if (k, d, num_items) in all_results]
            ...
            for ext_idx in range(num_ext):
                ...
                for d_idx, degree in enumerate(avail_degrees):
                    ...
                    ax.plot(e_line, y, color=degree_colors[d_idx], linewidth=2.0,
                            label=f'deg {degree}', zorder=3)
...
def plot_welfare_distributions(all_results, fig_dir):
    ...
    col_specs = [
        ...
        ('vcg_adv_welfare', 'vcga_adv_welfare', 'ra_adv_welfare', 'Advertiser Welfare (v)'),
        ('vcg_ext_welfare', 'vcga_ext_welfare', 'ra_ext_welfare', 'Externality Welfare (e)'),
    ]
    ...
    for (k, degree, num_items), records in all_results.items():
        ...
        for row_idx, rec in enumerate(records_sorted):
            ...
            for col_idx, (vcg_key, vcga_key, ra_key, col_title) in enumerate(col_specs):
                ...
                ax.hist(vcga_vals, bins=bins, alpha=0.5, color='firebrick', label='VCGA')
...
def parse_args():
    ...
    # Sweep dimensions
    p.add_argument('--degrees',   default='1',
                   help='Comma-separated polynomial degrees, e.g. "1,2,3" (default: 1)')
    p.add_argument('--k-values',  default='1',
                   help='Comma-separated k values, e.g. "1,2" (default: 1)')
    p.add_argument('--ext-min',   type=float, default=0.01,
                   help='Minimum externality cost (default: 0.01)')
    p.add_argument('--ext-max',   type=float, default=100.0,
                   help='Maximum externality cost (default: 100.0)')
    p.add_argument('--num-ext',   type=int,   default=8,
    ...
    p.add_argument('--action-cost',  type=float, default=1.0,
...
def main():
    ...
    if args.log_scale:
        ext_values = list(np.logspace(
            np.log10(args.ext_min), np.log10(args.ext_max), args.num_ext))
    else:
        ...
        for n_idx, num_items in enumerate(num_items_list):
            ...
            for ext_idx, ext_cost in enumerate(ext_values):
                ...
                for degree in degrees_ordered:
                    ...
                    ga_seed     = args.seed + ext_idx + degree * 1000 + k * 100000 + n_idx * 10000000
    ...
    plot_tau_degree_comparison(all_results, degrees_ordered, fig_dir, k_values, num_items_list)
