...
def _run_one_sgd(advertisers, polynomial_degree, k, beta0, eps_vec,
    ...
    for t in range(1, num_iterations + 1):
        ...
        for ki in range(d):
            delta = np.zeros(d)
            delta[ki] = eps_vec[ki]
            grad[ki] = (batch_welfare(beta + delta, batch) - batch_welfare(beta - delta, batch)) / (2 * eps_vec[ki])
...
def run_sgd_search(advertisers, polynomial_degree, k=1, num_iterations=500,
    ...
    if num_restarts == 1:
        ...
        intercepts = [v_min + i * (v_max - v_min) / (num_restarts - 1) for i in range(num_restarts)]
...
def print_usage():
    ...
    print("  --eps FLOAT                Optional. Finite-difference base step size. Defaults to 0.1.")
    print("  --num-restarts INT         Optional. SGD restarts sweeping intercept. Defaults to 5.")
