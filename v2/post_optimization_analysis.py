...
def _welfare_from_sets(bidder_sets, tau_coeffs, k):
    ...
    for bs in bidder_sets:
        ...
        if len(adm_v) == 0:
            vcga_w.append(0.0); vcga_av.append(0.0); vcga_ev.append(0.0)
            pa_pay.append(0.0)
        else:
    ...
    return {
        'vcg_welfare': vcg_w,   'vcg_adv_welfare': vcg_av,   'vcg_ext_welfare': vcg_ev,
        'vcga_welfare': vcga_w, 'vcga_adv_welfare': vcga_av, 'vcga_ext_welfare': vcga_ev,
        'pa_payments': pa_pay,
        'ra_welfare': ra_w,     'ra_adv_welfare': ra_av,     'ra_ext_welfare': ra_ev,
    }
...
def recompute_welfare_consistent(all_results, data_path, base_seed, num_test_auctions):
    ...
    print(f"Recomputed welfare for {sum(len(v) for v in all_results.values())} cells "
...
def _e_range(all_e, percentile=95):
    return float(np.percentile(all_e, 100 - percentile)), float(np.percentile(all_e, percentile))
...
_PALETTE = ['tab:purple', 'tab:orange', 'tab:green', 'tab:red', 'tab:blue',
...
def _find_rec(all_results, k, degree, num_items, ext_cost):
    ...
    matched = [r for r in all_results[key] if abs(r['externality_cost'] - ext_cost) <= tol]
...
def plot_welfare_distributions_grouped(all_results, fig_dir):
    ...
    for (k, degree, num_items), records in all_results.items():
        ...
        avail_cols = [s for s in col_specs if all(key in first for key in (s[0], s[1], s[2]))]
        if not avail_cols:
            print(f"  Skipping welfare distributions for k={k} deg={degree} n={num_items}: "
                  f"missing adv/ext breakdown keys (rerun sweep to generate them)")
            continue

        num_ext = len(records_sorted)
        ncols_plot = len(avail_cols)
        fig, axs = plt.subplots(num_ext, ncols_plot,
                                figsize=(5 * ncols_plot, 3.5 * num_ext),
                                squeeze=False)

        for row_idx, rec in enumerate(records_sorted):
            ext_cost = rec['externality_cost']
            for col_idx, (vcg_key, vcga_key, ra_key, col_title) in enumerate(avail_cols):
...
def plot_tau_degree_comparison(all_results, degrees, fig_dir, k_values, num_items_list,
    ...
    for k in k_values:
        for num_items in num_items_list:
            avail_degrees = [d for d in degrees
                             if (k, d, num_items) in all_results
                             and all_results[(k, d, num_items)]]
            if not avail_degrees:
                continue

            degree_colors = [_PALETTE[i % len(_PALETTE)] for i in range(len(avail_degrees))]
            ...
            for ext_idx in range(num_ext):
                ...
                for d_idx, degree in enumerate(avail_degrees):
                    ...
                    delta_w = float(np.mean(rec['vcga_welfare'])) - float(np.mean(rec['vcg_welfare']))
                    ax.plot(e_line, y, color=degree_colors[d_idx], linewidth=2.0, zorder=3,
...
def plot_tau_by_n(all_results, fig_dir, degree=1, empirical=False, percentile=95):
    ...
    for k in k_values:
        ...
        for ec_key in sorted(ec_map):
            ...
            fig, axs = plt.subplots(1, len(num_items_list),
...
def plot_tau_grid(all_results, fig_dir, max_fns=200, zoomed=False, percentile=99):
    ...
    for (k, degree, num_items), records in all_results.items():
        ...
        for idx, rec in enumerate(records_sorted):
            ...
            for coeffs in fns:
                ax.plot(e_line, [_eval_poly(coeffs, e) for e in e_line],
                        color='#555555', alpha=0.35, linewidth=0.7, zorder=2)
        ...
        fig.tight_layout()
        fname = (f'tau_grid_zoomed_k{k}_deg{degree}_n{num_items}.png' if zoomed
...
def plot_tau_grid_by_degree(all_results, fig_dir, max_fns=200, zoomed=False, percentile=99):
...
def plot_penalty_grid(all_results, fig_dir, percentile=95, empirical=False):
    ...
    for (k, degree, num_items), records in all_results.items():
        ...
        for idx, rec in enumerate(records_sorted):
            ...
            R     = _participant_penalty_curve(e_line, rec['tau_coeffs'], rec['pa_payments'])
            neg_e = -e_line
            ...
            if not legend_done:
                ax.legend([p1, p2, p3],
                          ['tau (optimal threshold)', 'r(e) (participant penalty)',
                           'R(e) (recipient penalty)'], loc='best')
                legend_done = True
...
def plot_welfare_comparison(all_results, fig_dir, empirical=False, show_ci=True):
    ...
    for (k, degree, num_items), records in all_results.items():
        ...
        vcg_m  = np.array([t[0] for t in _vcg]);  vcg_ci  = np.array([t[1] for t in _vcg])
        vcga_m = np.array([t[0] for t in _vcga]); vcga_ci = np.array([t[1] for t in _vcga])
        ra_m   = np.array([t[0] for t in _ra]);   ra_ci   = np.array([t[1] for t in _ra])

        use_log_x = (len(ext_costs) > 1 and min(ext_costs) > 0
                     and max(ext_costs) / min(ext_costs) > 100)

        fig, ax = plt.subplots(figsize=(8, 5))

        if show_ci:
            ax.errorbar(ext_costs, vcg_m,  yerr=vcg_ci,  marker='o',
                        label='VCG (unconstrained)', color='steelblue',
                        capsize=3, capthick=1.0, elinewidth=0.9)
            ax.errorbar(ext_costs, vcga_m, yerr=vcga_ci, marker='s',
                        label='Participant Audit',   color='firebrick',
                        capsize=3, capthick=1.0, elinewidth=0.9)
            ax.errorbar(ext_costs, ra_m,   yerr=ra_ci,   marker='^',
                        label='Recipient Audit',     color='seagreen',
                        capsize=3, capthick=1.0, elinewidth=0.9)
            ...
            for ki, k in enumerate(k_values):
                ...
                for num_items in num_items_list:
                    ...
                    if cell not in all_results:
                        continue
                    matched = [r for r in all_results[cell]
                               if abs(r['externality_cost'] - ext_cost) <= tol]
                    if not matched:
                        continue
                    rec = matched[0]
                    m, ci = _welfare_ci(rec['vcg_welfare']);  vcg_ms.append(m);  vcg_cis.append(ci)
                    m, ci = _welfare_ci(rec['vcga_welfare']); vcga_ms.append(m); vcga_cis.append(ci)
                    m, ci = _welfare_ci(rec['ra_welfare']);   ra_ms.append(m);   ra_cis.append(ci)
                ...
                ax.errorbar(ns_avail, vcg_ms,  yerr=vcg_cis,  marker='o',
                            label='VCG (unconstrained)',  color='steelblue',  **ebar_kw)
                ax.errorbar(ns_avail, vcga_ms, yerr=vcga_cis, marker='s',
                            label='Participant Audit',    color='firebrick',  **ebar_kw)
                ax.errorbar(ns_avail, ra_ms,   yerr=ra_cis,   marker='^',
                            label='Recipient Audit',      color='seagreen',   **ebar_kw)
...
def parse_args():
    ...
    p.add_argument('--figures-subdir', default='figures',
    ...
    p.add_argument('--num-test-auctions', type=int, default=2000,
...
def run_one_dir(sweep_dir, figures_subdir, data_path=None, base_seed=1234,
    ...
    if data_path is not None:
        ...
        recompute_welfare_consistent(all_results, data_path, base_seed, num_test_auctions)
