...
def latex_tweet_summary_stats(path):
    ...
    show_stats = [c for c in ['mean', 'std', 'min', 'p25', 'p50', 'p75', 'max'] if c in df.columns]
    ...
    return _wrap_table(
        ...
        notes=(r'All rate variables (per month) computed as lifetime total divided by '
...
def latex_tweet_correlations(path):
    ...
    col_spec = 'l' + 'r' * n
    short = [_SHORT_METRIC_LABELS.get(c, c.replace('_', r'\_')) for c in cols]
    ...
    for i, (metric, corr_row) in enumerate(df.iterrows()):
        label = f'({i+1})~' + _SHORT_METRIC_LABELS.get(str(metric), str(metric).replace('_', r'\_'))
        cells = []
...
def latex_note_classification(path):
    ...
    for _, row in df.iterrows():
        label = _CLASSIFICATION_LABELS.get(str(row['classification']),
...
def latex_rating_distribution(path):
    ...
    rows = []
    for _, row in df.iterrows():
        label = _HELPFULNESS_LABELS.get(str(row['helpfulnessLevel']),
                                        str(row['helpfulnessLevel']))
        rows.append(f'{label} & {int(row["count"]):,} & {_pct(row["pct"])}')
    total = df['count'].sum()
    rows.append(rf'\midrule Total & {int(total):,} & 100.0\%')
...
def latex_note_category_frequency(path):
    ...
    rows = []
    for _, row in df.iterrows():
        label = str(row['label']) if 'label' in df.columns else str(row['category']).replace('_', r'\_')
        rows.append(f'{label} & {int(row["count"]):,} & {_pct(row["pct"])}')
...
def latex_category_rating_totals(path):
    ...
    for _, row in df.iterrows():
        label = str(row['label']) if 'label' in df.columns else str(row['category']).replace('_', r'\_')
        agree_rate = f'{float(row["agree_rate"]):.2f}' if 'agree_rate' in df.columns else '--'
        rows.append(
    ...
    return _wrap_table(
        ...
        notes=(r'\textit{Agree} = Helpful ratings; \textit{Disagree} = Not-Helpful ratings. '
...
def parse_args():
    ...
    p.add_argument('--input-dir', default='output/descriptive_stats/tables',
