...
def print_usage():
    ...
    print("  --num-parents-mating INT     Optional. Parents per generation. Defaults to 15.")
    print("  --mutation-probability FLOAT Optional. Per-gene mutation rate. Defaults to 0.25.")
    print("  --range-multiplier FLOAT     Optional. Gene range = ±mult×std_v/mean(|e|^k). Defaults to 3.0.")
...
def parse_args(argv):
    ...
    while i < len(argv):
        ...
        if arg == '--help':
            ...
            i += 1; args['num_generations'] = int(argv[i])
        elif arg == '--sol-per-pop':
            i += 1; args['sol_per_pop'] = int(argv[i])
        elif arg == '--num-parents-mating':
            i += 1; args['num_parents_mating'] = int(argv[i])
        elif arg == '--mutation-probability':
            i += 1; args['mutation_probability'] = float(argv[i])
        elif arg == '--range-multiplier':
            i += 1; args['range_multiplier'] = float(argv[i])
        elif arg == '--id':
